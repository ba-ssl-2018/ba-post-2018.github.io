...
각각의 데이터 포인트마다, 자신을 제외한 인접한 k개의 데이터 포인트와의 거리를 계량함으로써 novelty score을 계산하고, 이를 근거로 outlier를 걸러내는 것을 골조로 하는 방식입니다. <br></br>
이 때, 거리를 계량하는 방식에 따라
* max 거리
...
1. def k_neighbor_novelty_score(from_point, to_points): <br></br>
...
각각의 데이터 포인트마다, 가장 가까운 centroid까지의 거리를 계량함으로써 novelty score을 계산하고, 이를 근거로 outlier를 걸러내는 것을 골조로 하는 방식입니다(centroid란, k-means clustering, 즉, EM 알고리즘의 수행 결과로써 나온 centrods를 의미합니다). <br></br>
이 때, 거리를 계량하는 방식에 따라
* 절대 거리
* 상대 거리
로 세분화 될 수 있습니다.
...
1. def closest_centroid_novelty_score(from_point, to_centroids): <br></br>
...
2. 1000개의 숫자데이터(각각이 pixels를 담고 있는 record)를 로드하고, k-means clustering 시킵니다. 이 때, 숫자 라벨의 종류는 10가지(0~9) 이므로 총 10개의 clusters가 생기도록 합니다.
